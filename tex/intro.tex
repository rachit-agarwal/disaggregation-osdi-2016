...
We answer these questions in the context of \rc{{\em nine}} diverse workloads spanning \rc{six} popular open-source systems --- off-disk batch processing using Hadoop, in-memory batch processing using Spark, distributed graph processing using GraphLab, extremely optimized graph processing using Timely dataflow~\cite{timely-dataflow, timely-dataflow1}, point queries with memcached~\cite{memcached} \rc{and HERD~\cite{herd}}, and an optimized in-memory SQL engine SparkSQL. Our key findings are:
