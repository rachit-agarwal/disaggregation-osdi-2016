...
\vspace{-0.05in}
\rqc{
Our study so far has focused on characterizing the network requirements for \dis that would lead to little or no performance degradation in existing applications. However, in longer term, one might expect to re-architect existing applications to exploit \dis for improved performance. This is possible due to two factors enabled by \dis: (1) better resource utilization via resource multiplexing{\footnote{As argued in~\cite{hotnets, reiss}, }}; and (2) availability of higher density memory architectures~\cite{ddcHwDesign1}.
}
