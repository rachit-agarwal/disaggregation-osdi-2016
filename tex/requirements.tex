...
We elaborate briefly on our choice to take these applications as is, rather than seek to optimize them for \dis. Our focus in this paper is on understanding whether and why networking might gate the deployment of \dis.  For this, we are interested in the degradation that applications might suffer if they were to run in a disaggregated context. Disaggregation will, by definition, not degrade applications that have been rewritten to exploit \dis.
%(since their baseline performance is already based on a disaggregated system). 
In contrast, a legacy (i.e., server-centric) application is likely to experience degradation and in fact represents the worst-case in terms of potential degradation (since a legacy implementation of an application $A$ running on a disaggregated datacenter will always perform worse than a rewritten version of $A$ that is designed to exploit disaggregation).

Thus focusing on legacy applications gives us a lower bound on the network requirements needed for disaggregation. Clearly, if new networking technologies exceed this lower bound, then all applications (legacy and `native' \dis) will benefit. Similarly, new programming models designed to exploit disaggregation can only improve the performance of all applications. The question of how to achieve improved performance through new technologies and programming models is an interesting one but beyond the scope of our effort and hence one we leave to future work. 
}
...
\begin{figure*}[t]
    ...
    \caption{\small{\rqc{Performance degradation of applications is strongly correlated with the swap memory bandwidth and the overall memory bandwidth utilization.}}}
    \label{fig:bandwidths11}
\end{figure*}
%

\paragraphb{Emulating remote memory} 
When running the above workloads, we emulate remote memory accesses by implementing a special swap device that is backed by physical memory rather than a disk. 
...
\pg{As expected, we found that Spark Streaming is memory efficient, and it does not use too much remote memory. Hence its application level degradation is near zero when latency is injected. We therefore did not plot it in Figure \ref{fig:latb}.}
...
We partially answer these questions using Figure~\ref{fig:bandwidths11}, which plots the performance degradation of the above nine workloads against their swap and memory bandwidth{\footnote{\rqc{These measurements were performed using Intel's Performance Counter Monitor software~\cite{intel-mem-count} to read the uncore performance counters in each node in our local cluster, which reflects the number of bytes written to and read from the integrated memory controller on each CPU. We confirmed using benchmarks designed to saturate memory bandwidth~\cite{mem-band-bench} that we could observe memory bandwidth utilization numbers approaching the reported theoretical maximum. As further validation, we verified that our Spark SQL measurement is consistent with prior work~\cite{georgeporter}.}}}. Figure~\ref{fig:swapband} and~\ref{fig:memband} show that the performance degradation of applications is strongly correlated with the swap and the memory bandwidth utilized by the application. While the correlation with swap bandwidth is rather intuitive, the difference between the two figures suggests that the performance degradation also depends on ``locality'', that is, the distribution of data accesses across the range of memory addresses. Measuring these access patterns and other contributing factors to performance degradation is an interesting avenue of future research.
