...
In order to better understand the memory bandwidth utilization of each application, we used Intel's Performance Counter Monitor software~\cite{intel-mem-count} to read the uncore performance counters in each node in our local cluster, which reflects the number of bytes written to and read from the integrated memory controller on each CPU. The resulting correlation is in Figure~\ref{fig:memband}. 
To validate our measurement methodology, we confirmed that we could observe memory bandwidth utilization numbers near the theoretical maximums reported by Intel using benchmarks designed to saturate memory bandwidth~\cite{mem-band-bench}.
\rc{PG: Our Spark SQL numbers are also consistent with the measurement did in ~\cite{georgeporter}}
}
