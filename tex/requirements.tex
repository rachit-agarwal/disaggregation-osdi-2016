...
One might ask \emph{why} we see the above requirements -- i.e., what characteristic of the applications that we evaluate led to the specific bandwidth and latency requirements we report? An understanding of these characteristics could also allow us to generalize our findings to other applications. 
...
rather intuitive, the difference between the two figures suggests that the performance degradation also depends on ``locality'', that is, the distribution of data accesses across the range of memory addresses. Measuring these access patterns and other contributing factors to performance degradation is an interesting avenue of future research.
